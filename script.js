...
const RECOMMENDATION_BEACH_NAMES = { gwangalli: "광안리해수욕장", haeundae: "해운대해수욕장", songjeong: "송정해수욕장", songdo: "송도해수욕장", dadaepo: "다대포해수욕장", ilgwang: "일광해수욕장", imrang: "임랑해수욕장", yeongdo: "영도 바다", cheongsapo: "청사포", gijang: "기장 바다" };
...
form.addEventListener("submit", (event) => {
  ...
  const { recommend, localize, tagLabel } = window.BacochuRecommendations;
  ...
  resultContainer.innerHTML = `
    ...
    <div class="recommendation-list">${recommendations.map(({ item, score }, rank) => `
      <article class="recommendation-card">
        ...
        <p class="recommendation-traits">${t("추천 대상 또는 여행 성향")}: ${item.tags.slice(0, 5).map((tag) => escapeHtml(tagLabel(tag, language))).join(" · ")}</p>
...
function openCourseDetail(id) {
  ...
  const tagLabel = window.BacochuRecommendations?.tagLabel;
  const builtInCourse = recommendation && {
    ...
    companion: recommendation.tags.slice(0, 3).map((tag) => tagLabel(tag, language)).join(" · "), mood: localize(recommendation.timeLabel, language),
